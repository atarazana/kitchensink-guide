...
Additionally you have make the next change (this is around line 20):
...
[source,yaml,subs="attributes+,+macros"]
----
- name: image-registry.openshift-image-registry.svc:5000/{cicd-namespace}-{username}/kitchensink:latest
...
Don't forget to make the following change (this is around line 20):
