...
In a nutshell the `assemble` script:
...
* Once at runtime, the `run` script comes into play and it will look for execution hooks ()

En ejecución se ejecutará el fichero `postconfigure.sh` antes de arrancar JBoss
