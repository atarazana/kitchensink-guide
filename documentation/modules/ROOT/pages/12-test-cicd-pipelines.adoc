...
----
curl -X POST -k  http://$(oc get route/el-kitchensink-ci-pl-push-{git-provider}-listener  -n $CICD_NAMESPACE -o jsonpath='{.spec.host}') \
...
export CICD_NAMESPACE="kitchensink-cicd-$\{USERNAME}"
oc logs deployment/el-kitchensink-ci-pl-push-{git-provider}-listener -n ${CICD_NAMESPACE} | grep dummy
----
...
[source,bash, subs="+attributes"]
----
curl -X POST -k  http://$(oc get route/el-kitchensink-cd-pl-pr-{git-provider}-listener  -n $CICD_NAMESPACE -o jsonpath='{.spec.host}') \
 -H 'X-GitHub-Event: dummy' -H 'X-GitHub-Event-Type: dummy' -H 'X-Gitea-Event: dummy' -H 'X-Gitea-Event-Type: dummy' \
 -H 'Content-Type: application/json' \
 --data '{ "dummy": "dummy"}'
----
...
export USERNAME="{username}"
export CICD_NAMESPACE="kitchensink-cicd-$\{USERNAME}"
oc logs deployment/el-kitchensink-cd-pl-pr-{git-provider}-listener -n ${CICD_NAMESPACE} | grep dummy
----